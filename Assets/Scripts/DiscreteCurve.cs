...
public class DiscreteCurve : MonoBehaviour {
    ...
    public void SolveImpulses(int numSegments)
    {
        ...
        foreach (GRBVar sigma_i in sigma)
        {
            Debug.Log(sigma_i.Get(GRB.StringAttr.VarName) + " = " + sigma_i.Get(GRB.DoubleAttr.X));
        }
        ...
        for (int i = 0; i < absDiffsMinus.Count; i++)
        {
            float plus = (float)absDiffsPlus[i].Get(GRB.DoubleAttr.X);
            float minus = (float)absDiffsMinus[i].Get(GRB.DoubleAttr.X);
            Debug.Log("Abs diff " + i + " = (" + plus + ", " + minus + ")");
        }
